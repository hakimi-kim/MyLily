<script lang="ts">
  ...
  const STORAGE_KEY = "mylily.memories.v1";
  ...
  onMount(() => {
    try {
      ...
      if (raw) {
        ...
        memories = parsed;
        prevStageIdx = getStage(parsed.length).idx;
      }
    } catch {}
    loaded = true;
  });
  ...
  let stageInfo = $derived(getStage(memories.length));
  ...
  let progress = $derived(
    nextStage
      ? Math.min(100, ((memories.length - currentStage.min) / (nextStage.min - currentStage.min)) * 100)
      : 100
  );
 
  function plant(e: SubmitEvent) {
    e.preventDefault();
    ...
    const mem: Memory = {
      ...
      date,
      hue: 320 + Math.floor(Math.random() * 60) - 30,
      createdAt: Date.now(),
    };
    const next = [mem, ...memories];
    memories = next;
    justPlantedId = mem.id;
    setTimeout(() => (justPlantedId = null), 1400);
 
    const newIdx = getStage(next.length).idx;
        ...
        <div>
          <div class="flex items-baseline justify-between mb-2">
            <p class="text-sm text-[oklch(0.5_0.05_30)]">
              <span class="font-serif text-2xl text-[oklch(0.35_0.08_25)]">{memories.length}</span>
              {memories.length === 1 ? "memory" : "memories"} planted
            </p>
            {#if nextStage}
              <p class="text-xs text-[oklch(0.55_0.06_30)]">
                {nextStage.min - memories.length} until {nextStage.emoji} {nextStage.name}
              </p>
            {:else}
...
{#if openForm}
  ...
  >
    ...
    >
      ...
      <label class="block text-xs uppercase tracking-wider text-[oklch(0.5_0.05_30)] mb-1">Title</label>
      <input
        autofocus
        bind:value={title}
        required
        class="w-full px-4 py-2.5 rounded-xl bg-[oklch(0.97_0.015_85)] border border-[oklch(0.88_0.03_30)] outline-none focus:border-[oklch(0.65_0.14_20)] mb-4"
        placeholder="Our first sunset"
      />
 
      <label class="block text-xs uppercase tracking-wider text-[oklch(0.5_0.05_30)] mb-1">Description</label>
      <textarea
        bind:value={description}
        rows="4"
        class="w-full px-4 py-2.5 rounded-xl bg-[oklch(0.97_0.015_85)] border border-[oklch(0.88_0.03_30)] outline-none focus:border-[oklch(0.65_0.14_20)] mb-4 resize-none"
        placeholder="Tell the story of this moment..."
      ></textarea>
 
      <label class="block text-xs uppercase tracking-wider text-[oklch(0.5_0.05_30)] mb-1">Date</label>
      <input
        type="date"
        bind:value={date}
        class="w-full px-4 py-2.5 rounded-xl bg-[oklch(0.97_0.015_85)] border border-[oklch(0.88_0.03_30)] outline-none focus:border-[oklch(0.65_0.14_20)] mb-6"
      />
      ...
      <div class="flex gap-2 justify-end">
        <button
          type="button"
          onclick={() => (openForm = false)}
          class="px-5 py-2.5 rounded-xl text-[oklch(0.45_0.05_30)] hover:bg-[oklch(0.95_0.02_60)] cursor-pointer"
        >
          Cancel
        </button>
        <button
          type="submit"
          class="px-5 py-2.5 rounded-xl bg-[oklch(0.65_0.14_20)] text-[oklch(0.98_0.01_85)] font-medium hover:bg-[oklch(0.6_0.16_20)] cursor-pointer"
        >
          Plant ✿
        </button>
      </div>
